...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Walk-forward rolling validation for low-turnover ADX+DI+V2 strategies."
    )
    parser.add_argument("--input-csv", required=True)
    parser.add_argument("--v2-validation-csv", required=True)
    parser.add_argument("--output-dir", default="validation_outputs/rolling_adxdi_v2_low_turnover_run")
...
def simulate_config(df: pd.DataFrame, config: StrategyConfig, fee_rate: float) -> Dict[str, float]:
...
def main() -> None:
    ...
    window_results.to_csv(output_dir / "rolling_adxdi_v2_low_turnover_window_results.csv", index=False)
    ...
    selected_counts.to_csv(output_dir / "rolling_adxdi_v2_low_turnover_selected_counts.csv", index=False)
    ...
    fixed_results.to_csv(output_dir / "rolling_adxdi_v2_low_turnover_fixed_benchmarks.csv", index=False)
    ...
    (output_dir / "rolling_adxdi_v2_low_turnover_summary.json").write_text(
